...
async def main(message: cl.Message):
    ...
    msg = await cl.Message(content=f"**Answer:** {answer}").send()

    # Add sources to the widget
    if sources:
        sources_text = "\n\n".join([f"**Source {i}:** {doc.page_content}" for i, doc in enumerate(sources, 1)])
        await cl.Text(content="Retrieved Sources", name="Sources", value=sources_text).send(for_id=msg.id)
