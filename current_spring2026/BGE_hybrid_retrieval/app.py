#!/usr/bin/env python3
"""
Streamlit application for the BGE-M3 hybrid RAG system.
...
Environment variables (set in .env or the shell):
    ...
    BGE_INDEX_DIR             (optional — path to indexes directory, defaults to indexes/1940)
"""
...
st.set_page_config(
    page_title="BPL Hybrid RAG — BGE-M3",
    page_icon="📚",
    layout="wide",
)
...
def initialize_session() -> None:
    ...
    if "indexes" not in st.session_state:
        ...
        try:
            ...
            st.error(f"Failed to load indexes from {index_dir}: {e}")
            st.stop()
...
def main() -> None:
    st.title("📚 BPL Digital Commonwealth — BGE-M3 Hybrid RAG")
    st.caption(
        "Hybrid chunk-level + title-level dense retrieval with Reciprocal Rank Fusion. "
        "Powered by BAAI/bge-m3 and local FAISS indexes."
    )
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching collections (chunk + title dense retrieval + RRF)…"):
    ...
    st.markdown("---")
    st.caption(
        "Built with BGE-M3 · FAISS · RRF · LangChain · Streamlit · OpenAI"
    )
